...
public class Lucene {
    ...
    // 定义索引库的位置
    private static final String INDEX_PATH = "F:/lucene/indexdata";
    @Test
    public void indexCreate1() throws IOException {
        ...
        StringField titile = new StringField("title", "谷歌地图之父跳槽FaceBook", Store.YES);
        doc.add(titile);
    ...
    public void indexCreate() throws IOException {
        ...
        TextField titile = new TextField("title", "传智播客java编程思想", Store.YES);
        doc.add(titile);

//		new TextField("title", "谷歌地图之父跳槽FaceBook", Store.YES);
        // TextField：创建索引 , 并提供分词，
        // StringField 创建索引 , 但不分词
        TextField content = new TextField("content", "谷歌地图之父跳槽FaceBook", Store.YES);
        doc.add(content);
...
//		indexWriter.addDocument(doc);
        ...
        for (Long i = 3L; i < 30; i++) {
            ...
            TextField content = new TextField("content", "鸟哥的JAVA技术私房菜" + i, Store.YES);
     ...
     */
    @Test
    public void searchIndex1() throws IOException, ParseException {

        //查询解析器对象  构造参数：1.搜索的目标字段名称   2.使用何种分词器对搜索的参数进行分析
        QueryParser queryParser = new QueryParser("title", new StandardAnalyzer());

        // 对搜索的参数进行解析  解析后得到Query对象
        Query query = queryParser.parse("传智播客java编程思想");
...
//		Term term = new Term("查询的目标字段", "查询的参数");
    ...
    public void wildcardQueryTest() {
        Term term = new Term("content", "faceboo?");
    ...
    public void fuzzyQueryTest() {
        Term term = new Term("content", "face");
...
//		 NumericRangeQuery.newLongRange("搜索的目标字段", 起始范围, 结束范围, 是否包含最小, 是否包含最大);
    ...
    public void booleanQueryTest() {
        ...
        NumericRangeQuery<Long> rangeQuery = NumericRangeQuery.newLongRange("id", 5L, 10L, true, true);
        ...
        //模糊查询  返回一条
        query.add(fuzzyQuery, Occur.MUST);
